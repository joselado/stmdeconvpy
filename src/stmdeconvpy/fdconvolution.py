...
#          if not 0<=(j+wp)<n: continue
          if wp<0: yi = y2[0] ; di = yfd[0]
          elif wp>=n: yi = y2[n-1] ; di = yfd[n-1]
          else: yi = y2[wp] ; di = yfd[wp]
          if j>=n: yj = y1[n-1] ; dj = yfd[n-1]
          elif j<0: yj = y1[0] ; dj = yfd[0]
          else: yj = y1[j] ; dj = yfd[j]
#          d = yfd[j]
